...
\myFigure{MVVM}{Den implementerede arkitektur i applikationen. View'et er adskildt fra modellen med en ViewModel. Modellen er delt op i flere klasser for at opfylde SOLID\citep{RefWorks:10} pricipperne hvor det giver mening i applikationen}{fig:mvvm}{0.6}
...
\myFigure{CCAClassDiagram}{Klasse diagram over CrustCrawler applikationen. Klasserne af lavet ud fra SOLID principperne, og afhænger derfor ikke af hinanden med af interfaces. Hver klasse har også kun ét ansvar de skal sørge for. Applikationen er også åben for udvidelser, ved at der fx. kan tilføjes nye poses, uden der skal ændres i allerede eksisterende klasser. Der skal blot tilføjes et interface der nedarver fra IPoseHandlerMkI, hvor der kan tilføjes nye poses til.}{fig:cd}{1}
