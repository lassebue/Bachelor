...
\myFigure{MVVM}{Den implementerede arkitektur i applikationen. View'et er adskildt fra modellen med en ViewModel. Modellen er delt op i flere klasser for at opfylde SOLID pricipperne hvor det giver mening i applikationen}{fig:mvvm}{0.8}
\todokr{lave reference i figurteksten til design bogen og ret GUI til View}
...
\label{sec:ktc}
Som nævnt tidligere foregår alt kommunikation med CrustCrawleren igennem matlab klassen. Her sørges der for at kalde funktioner i en MATLAB Automation server\footnote{Fremover nævnt som MATLAB server} der bliver åbnet under initieringen af matlab klassen. På sekvens diagrammet på figur \ref{fig:sd} ses de kald der bliver foretaget i tilfældet at kloen på CrustCrawleren skal åbnes.
...
\myWrapFigure{USB2Dynamixel}{USB2Dynamixel bruges for at lave en seriel port fra en af computerens USB porte}{hej}{0.5}{r}

CrustCrawleren tilkobles computeren gennem en USB2Dynamixel, se figur \ref{fig:usb2d}. USB2Dynamixel er brugt for at lave en seriel port igennem computerens USB port. Til denne er en driver og et .dll bibliotek, med en tilhørende headerfil. Dynamixel.dll og dynamixel.h. Disse filer loades begge i MATLAB koden, hvor biblioteket giver mulighed for at kalde nogle forskellige funktioner som CrustCrawleren kan reagere på. Driveren er "USB2Dynamixel" [\cite{usb2dynamixel}] \todokr{lav reference til manual hvis denne laves}

%\myFigure{USB2Dynamixel}{Figurtekst}{fig:usb2d}{0.3}

\section{Delkonklusion}
Det er nu muligt som bruger med denne applikation og en Myo at genkende håndrikkene åben hånd og lukket hånd, vha. en trænet machine learning model. Genkendelsen anvendes på en CrustCrawler robot arm, der åbnet og lukkes alt efter genkendelsen.